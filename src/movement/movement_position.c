...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/03/18 22:25:41 by audobnai          #+#    #+#             */
/*   Updated: 2026/03/18 22:37:06 by audobnai         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_vector			calc_move_direction(
							double angle,
							t_move_intent move_intent);
static t_move_intent	calc_move_intent(
							const t_controls_state *controls);
static void				try_move_x(
							t_scene *scene,
							t_position *next_position,
							t_vector move_direction,
							double move_distance);
static void				try_move_y(
							t_scene *scene,
							t_position *next_position,
							t_vector move_direction,
							double move_distance);
...
)
{
	...
	const t_vector		move_direction = calc_move_direction(
			scene->player.angle, move_intent);
	const double		move_speed_per_second = 3;
	const double		move_distance = move_speed_per_second
		* frame_delta_seconds;
	t_position			next_position;
...
static t_vector	calc_move_direction(double angle, t_move_intent move_intent)
{
	...
	move_direction.x = move_intent.y * y_direction.x + move_intent.x
		* x_direction.x;
	move_direction.y = move_intent.y * y_direction.y + move_intent.x
		* x_direction.y;
...
)
{
	...
	const double	x_position_candidate = scene->player.pos.x
		+ move_direction.x * move_distance;
	double			x;
	t_point			point_top;
	...
	if (move_direction.x > 0.0)
		...
		x = x_position_candidate - hit_radius;
	point_top = (t_point){.x = (int)floor(x), .y = (int)floor(next_position->y
			- hit_radius)};
	point_bottom = (t_point){.x = (int)floor(x),
	...
	if (is_in_bounds(point_top, scene->map, scene->map_size)
		...
		&& !is_wall(point_top, scene->map)
		&& !is_wall(point_bottom, scene->map))
...
{
	const double	hit_radius = scene->player.hit_radius;
	const double	y_position_candidate = scene->player.pos.y
		+ move_direction.y * move_distance;
	double			y;
	t_point			point_left;
	t_point			point_right;
	...
	if (move_direction.y > 0.0)
		...
		y = y_position_candidate - hit_radius;
	point_left = (t_point){.x = (int)floor(next_position->x - hit_radius),
		.y = (int)floor(y)};
	point_right = (t_point){.x = (int)floor(next_position->x + hit_radius),
		.y = (int)floor(y)};
	if (is_in_bounds(point_left, scene->map, scene->map_size)
		&& is_in_bounds(point_right, scene->map, scene->map_size)
		&& !is_wall(point_left, scene->map)
		&& !is_wall(point_right, scene->map))
