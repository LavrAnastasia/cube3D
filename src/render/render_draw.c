...
static void	render_wall_fallback(
				...
				t_range range,
				t_color color);
static void	render_wall_segment(
				t_wall_column column,
				t_px_buffer *buffer);

void	render_vertical_segment(
	t_range range,
	size_t x,
	t_px_buffer *buffer,
	int color)
...
void	render_ceiling_and_floor(
	t_wall_column wall_column,
	size_t window_height,
	t_px_buffer *buffer,
	const t_palette *palette)
{
	render_vertical_segment((t_range){
		.start = 0, .end = wall_column.projection.range.start},
		wall_column.x,
		buffer,
		palette->ceiling);
	render_vertical_segment((t_range){
		.start = wall_column.projection.range.end, .end = window_height},
		wall_column.x,
		buffer,
		palette->floor);
}
...
void	render_wall(t_wall_column wall_column, t_px_buffer *buffer)
{
	...
	{
		render_wall_fallback(wall_column.x, buffer,
			wall_column.projection.range, wall_column.fallback_color);
		return ;
	}
...
static void	render_wall_segment(t_wall_column column, t_px_buffer *buffer)
{
	...
	{
		position_down_wall = ((double)y - column.projection.top)
			/ column.projection.height;
		texture_y = (int)floor(position_down_wall
				* column.sample.texture->size.height);
		texture_y = clamp(texture_y, 0, column.sample.texture->size.height - 1);
		color = pixels_get(&column.sample.texture->px, column.sample.texture_x,
				texture_y);
